...
import numpy as np
import yaml
from experiments.utils import get_result_dir, setup_logger
...
def get_args():
    ...
    parser.add_argument("--n-var", type=int, default=6, help="number of design variables")
    parser.add_argument("--n-obj", type=int, default=2, help="number of objectives")
    parser.add_argument("--n-init-sample", type=int, default=50, help="number of initial design samples")
    parser.add_argument("--n-iter", type=int, default=20, help="number of optimization iterations")
    parser.add_argument(
        "--ref-point", type=float, nargs="+", default=None, help="reference point for calculating hypervolume"
    )
    parser.add_argument(
        "--batch-size", type=int, default=20, help="size of the evaluated batch in one iteration"
    )
    parser.add_argument(
        "--pop-init-method",
        type=str,
        choices=["nds", "random", "lhs"],
        default="nds",
