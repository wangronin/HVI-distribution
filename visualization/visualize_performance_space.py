    ...
    for kk in range(n_algo):
        ...
        for step in list(set(data_list[kk]["iterID"])):
            ...
            trace_dict = dict(
                ...
                hoverinfo="text",
                marker=dict(color="rgba(0, 0, 255, 0.8)", size=3 if n_obj == 2 else 2),
            )
            if n_obj > 2:
        ...
        for stepIndexes in stepTrace:
            ...
            iteration = dict(
                method="restyle", args=["visible", [False] * len(fig[kk].data)], label=str(j - 1)
            )
            j = j + 1
        ...
        sliders = [
            dict(
                active=int(len(steps)) - 1, currentvalue={"prefix": "Iteration: "}, pad={"t": 50}, steps=steps
            )
        ]
