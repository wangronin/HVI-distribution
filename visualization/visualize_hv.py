...
from experiments.arguments import get_vis_args
from experiments.utils import defaultColors, get_algo_names, get_problem_dir
...
def main():
    ...
    for i in range(n_algo):
        for j in range(n_seed):
            ...
            for k in [
                floor(0.25 * num_alg_eval),
                floor(0.5 * num_alg_eval),
                floor(0.75 * num_alg_eval),
                num_alg_eval,
            ]:
                df_boxplot = df_boxplot.append(
                    {
                        "SampleId": k,
                        "Hypervolume_indicator": df_HV_list[i][f"Hypervolume_indicator_{j + 1}"][
                            k + num_init_samples - 1
                        ],
                        "RunId": j,
                        "AlgorithmId": i,
                    },
    ...
    for i in range(n_algo):
        print(
            f"{args.problem}: {algo_names[i]} average HV indicator after {num_alg_eval} evaluations: {avgHV[i]}"
        )
    ...
    for i in range(n_algo):
        fig.add_trace(
            go.Scatter(
                ...
                y=avgHV_all[i],
                mode="lines",
                line=dict(color=defaultColors[i]),
            ),
            row=1,
            col=1,
        )
    ...
    fig.update_yaxes(
        title_text="Hypervolume Indicator",
        range=[minAvgHV - 0.05 * (maxAvgHV - minAvgHV), maxAvgHV + 0.05 * (maxAvgHV - minAvgHV)],
        row=1,
        col=1,
    )
    ...
    for i in range(n_algo):
        ...
        fig.add_trace(
            go.Box(
                y=algBox["Hypervolume_indicator"],
                x=algBox["SampleId"],
                name=algo_names[i],
    ...
    fig.update_yaxes(
        title_text="Hypervolume Indicator",
        range=[minHV - 0.05 * (maxHV - minHV), maxHV + 0.05 * (maxHV - minHV)],
        row=2,
        col=1,
    )
