...
class DistributionHVI(object):
    ...
    def density_eq7left(self, x, mean, variance, p, m, n):
        return  (x ** (2 * m - n - 1) *
                np.exp(-0.5 * (x ** 2 / variance[0] + p ** 2 / (x ** 2 * variance[1])) )
                )
    
    def method_without_taylor(self, mean, variance, truncatedLB, truncatedUB, p):
        L1, L2 = truncatedLB
        U1, U2 = truncatedUB
        ...
        D1, D2 = self.get_D(L1, U1, mean[0], variance[0] ** 0.5), self.get_D(L2, U2, mean[1], variance[1] ** 0.5)
        ...
        res = quad(
            self.original, alpha, belta, args=(mean, variance, p), 
            limit=int(1e7), epsabs=1e-30, epsrel=1e-10
        )[0]
        return res / (2 * np.pi * np.sqrt(variance[0]) * np.sqrt(variance[1]) * D1 * D2)


    def density_cosh(self, x, mean, variance, p, m, n):
        return 0.5 * (np.sqrt(variance[0] / variance[1] * p) **  (m - n/2)) * (
                np.exp(-p / np.sqrt(variance[0] * variance[1]) * np.cosh(x) + 
                   (m - n/2) * x) 
                ) 
    ...
    def computeTaylorSeries(self, p, mean, variance, truncatedLB, truncatedUB):
        ...
        return  term1 * np.nansum(faInCell)/(2 * np.pi * np.sqrt(variance[0]) * np.sqrt(variance[1]) * D1 * D2)
    ...
    def initialization(self, mu, ss, a):
        ...
        # initialized D:
        self.Dset = np.array([[("nan",) * m for j in range(0, num)] for i in range(0, num)], dtype=float)
        
        self.muPrime = np.array([[("nan",) * m for j in range(0, num)] for i in range(0, num)], dtype=float)
        
        # initialized HV for each cell
        self.HVIDistinCell = np.array([["nan" for j in range(0, num)] for i in range(0, num)], dtype=float)
        self.HVIDistinCell2 = np.array([["nan" for j in range(0, num)] for i in range(0, num)], dtype=float)
    ...
    def compute_HVI_dist(self, method, mu, ss, a):
        ...
        for i in range(0, self.num):
            for j in range(0, self.num - i):
                if i + j < self.num - 1 :
                    self.truncatedLB[i, j] = [self.q[0, self.num - j] - self.cellUB[i, j][0], 
                                              self.q[1, i] - self.cellUB[i, j][1]]
                    self.truncatedUB[i, j] = [self.q[0, self.num - j] - self.cellLB[i, j][0], 
                                              self.q[1, i] - self.cellLB[i, j][1]]
                else:
                    self.truncatedLB[i, j] = [0, 0]
                    self.truncatedUB[i, j] = [self.cellUB[i,j][0] - self.cellLB[i, j][0], 
                ...
                self.Dset[i, j] = [
                    norm.cdf(self.truncatedUB[i, j][k], loc=self.muPrime[i, j][k], scale=np.sqrt(ss[k]))
                    - norm.cdf(self.truncatedLB[i, j][k], loc=self.muPrime[i, j][k], scale=np.sqrt(ss[k]))
                    for k in range(0, self.m)
                ]
        
                self.gamma[i, j] = self.hvi(self.pf, self.r, self.cellUB[i, j]) \
                ...
                if method == 'Taylor':
                    ...
                    rst = quad(self.computeTaylorSeries,
                         0,
                         self.pInCell[i, j],
                         args=(muP, ss, [L1, L2], [U1, U2])
                         )[0]
                    
                    ProInCell = self.get_D(self.cellLB[i,j][0] , self.cellUB[i,j][0], mu[0], ss[0] ** 0.5) * \
                    ...
                    self.HVIDistinCell[i,j] = self.method_without_taylor(muP, ss, 
                                                                   [L1, L2], 
                                                                   [U1, U2], 
                                                                   self.pInCell[i, j]
                                                                   )
                    ...
                    hist_pdf = [self.method_without_taylor(muP, ss, [L1, L2], [U1, U2], p) for p in pvals]
