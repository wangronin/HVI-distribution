...
def MC_approximate(mean, variance, pf, r, n_mc, a, cellLB, cellUB):
    ...
    for i in range(n_mc):
        if (
            cellLB[0] < evaluated_points[i, 0] <= cellUB[0]
            and cellLB[1] < evaluated_points[i, 1] <= cellUB[1]
        ):
...
def func_v3(mean, variance, truncatedLB, truncatedUB, p):
    ...
    res = quad(
        original, alpha, belta, args=(mean, variance, p), limit=1000, epsabs=1e-30, epsrel=1e-10
    )[0]
...
for i in range(0, num):
    for j in range(0, num - i):
        truncatedLB[i, j] = [q[0, num - j] - cellUB[i, j][0], q[1, i] - cellUB[i, j][1]]

        truncatedUB[i, j] = [q[0, num - j] - cellLB[i, j][0], q[1, i] - cellLB[i, j][1]]
