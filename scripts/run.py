...
def main():
    ...
    parser.add_argument(
        "--n-inner-process", type=int, default=1, help="number of process can be used for each optimization"
    )
            ...
            for algo in args.algo:
                if algo == "nsga2":
                    ...
                    command = f"python main.py \
                        ...
                        --batch-size {args.batch_size} --n-iter {args.n_iter} \
                        --n-process {args.n_inner_process} \
                        --subfolder {args.subfolder} --log-to-file"  # --seed {args.seed}'  # kf
                ...
                if n_active_process >= args.n_process:
                    ...
                    if ret_code == signal.SIGINT:
                        exit()
                    print(
                        f"problem {ret_problem} algo {ret_algo} seed {ret_seed} done, time: "
                        + "%.2fs" % (time() - start_time)
                    )
                    n_active_process -= 1
