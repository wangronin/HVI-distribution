...
def integrand2(x, a, b, s, p):
    return 0.5 * (p * np.sqrt(a / b)) ** (s / 2) * np.exp(- p / np.sqrt(a * b) * np.cosh(x)  + x * s / 2)


def computeTaylorSeries(nTaylor,mu,s,p):
    #  mu: muHat[i,j]
    #  s: ssHat[i,j]
    faInCell = np.array([[('nan') for j in range(0,nTaylor)] for i in range(0,nTaylor)], dtype=float)
    for n in range(0,nTaylor):
        for m in range(0,n):
            # hao 
            faInCell[n,m] = pow( p, (n-m) )/ math.factorial(n) * \
...
# sorted pfr
q =  np.transpose(pfr[pfr[:,0].argsort(),])
...
for i in range(0,num):
    for j in range(0,num-i):           
        ...
        alpha_012[i,j][1] = (q[0,num-j] - cellLB[i,j][0]) / (cellUB[i,j][0]- cellLB[i,j][0]) - 1
        alpha_012[i,j][2] = (q[1,i] - cellLB[i,j][1]) / (cellUB[i,j][1]- cellLB[i,j][1]) - 1
        
        
        muHat[i,j][0] = (mu[0] - cellLB[i,j][0]) / (cellUB[i,j][0] - cellLB[i,j][0]) - alpha_012[i,j][1] - 1
        muHat[i,j][1] = (mu[1] - cellLB[i,j][1]) / (cellUB[i,j][1] - cellLB[i,j][1]) - alpha_012[i,j][2] - 1
        ssHat[i,j] = [np.sqrt(ss[k]) / (cellUB[i,j][k] - cellLB[i,j][k]) ** 2 for k in range(0,m) ]

        Dset[i,j] = [norm.cdf(1, loc=muHat[i,j][k], scale=np.sqrt(ssHat[i,j][k])) - \
                     norm.cdf(0, loc=muHat[i,j][k], scale=np.sqrt(ssHat[i,j][k])) for k in range(0,m) ]
        ...
        HVinCell[i,j] = (1/c[i,j] ) / (2 * np.pi * np.sqrt(ssHat[i,j][0] * ssHat[i,j][1]) * productInDset) * \
            np.exp(-0.5 * (muHat[i,j][0] ** 2 / ssHat[i,j][0] + muHat[i,j][1] ** 2 / ssHat[i,j][1])) * \
                computeTaylorSeries(nTaylor, muHat[i,j], ssHat[i,j], aInCell[i,j] ) # Taylor parts, @TODO: p is not done 
...
print(quad(integrand2, -500, np.log(np.sqrt(ss2)) - np.log(p * np.sqrt(ss1)), args=(ss1,ss2, nTaylor, p))) 
