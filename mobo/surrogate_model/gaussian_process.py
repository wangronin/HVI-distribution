    ...
    def __init__(self, n_var, n_obj, nu, **kwargs):
        ...
        for _ in range(n_obj):
            if nu > 0:
                ...
                main_kernel = RBF(
                    length_scale=np.ones(n_var), length_scale_bounds=(1e-10, 1e5)
                )
            ...
            gp = GaussianProcessRegressor(
                kernel=kernel, optimizer=constrained_optimization
            )
            self.gps.append(gp)
        ...
        for gp in self.gps:
            ...
            if calc_gradient:
                ...
                if std:
                    ...
                    K_Ki = (
                        K @ gp._K_inv
                    )  # gp._K_inv: shape (N_train, N_train), K_Ki: shape (N, 1, N_train)
                    dK_Ki = dK_T @ gp._K_inv  # dK_Ki: shape (N, n_var, N_train)

                    dy_var = -np.sum(
                        dK_Ki * K + K_Ki * dK_T, axis=2
                    )  # dy_var: shape (N, n_var)
                    dy_std = 0.5 * safe_divide(
                        dy_var, y_std
                    )  # dy_std: shape (N, n_var)
                    dS.append(dy_std)
            ...
            if calc_hessian:
                ...
                hd_N = (
                    d * np.expand_dims(np.eye(len(ell)), (0, 1))
                    - np.expand_dims(X_ - X_train_, 3) * dd
                )  # numerator
                ...
                if self.nu == 1:
                    ...
                    hK = (
                        ...
                        * (
                            -5 * d ** 2 * dd ** 2
                            + (1 + np.sqrt(5) * d) * (dd ** 2 + d * hd)
                        )
                    )

                else:  # RBF
                    hK = (
                        -sf2 * np.exp(-0.5 * d ** 2) * ((1 - d ** 2) * dd ** 2 + d * hd)
                    )
                ...
                if std:
                    ...
                    dK_Ki = np.expand_dims(
                        dK_Ki, 2
                    )  # dK_Ki: shape (N, n_var, 1, N_train)
                    hK_Ki = hK_T @ gp._K_inv  # hK_Ki: shape (N, n_var, n_var, N_train)

                    hy_var = -np.sum(
                        hK_Ki * K + 2 * dK_Ki * dK + K_Ki * hK_T, axis=3
                    )  # hy_var: shape (N, n_var, n_var)
                    hy_std = 0.5 * safe_divide(
                        hy_var * y_std - dy_var * dy_std, y_var
                    )  # hy_std: shape (N, n_var, n_var)
                    hS.append(hy_std)
